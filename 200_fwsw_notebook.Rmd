...
Adam's suggested I ask for an extension. That's what I should be doing! But I also want to know if the 12103 markers I've been using are filtered properly so I might run radiator on them. 
...
pop_afs<-do.call(rbind,lapply(pop.labs,function(pop,ful,cols){
  ...
  h<-hist(ful_afs$AltFreq,breaks=seq(0,0.5,0.01),main=pop,col = cols[pop],
          xlab = "Allele Frequency",ylab = "Frequency",freq = TRUE)
...
fsts<-plot_multiple_LGs(list_fsts = fst_dat,fst_name = "Corrected.AMOVA.Fst",bp_name="BP",chr_name="Chr",
                        lgs=lgs,plot_labs=list("TXFW vs TXCC","ALST vs FLSG","FLCC vs FLPB"),
