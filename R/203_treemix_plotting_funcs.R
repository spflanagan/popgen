		...
		for(i in 1:nrow(tmp)){
			...
			if(plotnames){
			  ...
			  if(!is.null(tip.order)){
			    label<-tip.order[tip.order[,1]==tmp[i,2],2]
			  }else { tmp[i,2] }
				text(tmp[i,]$x+disp, tmp[i,]$y, labels = label, adj = 0, cex = cex, col  = tcol, font = font)
			}
		}
	}
	else{
  ...
  for (i in 1:npop){
    for( j in 1:i){
      ...
      if (v < 0){
				...
				else{ #col = rgb(0, 0, 1, 0.1+0.9*(v/min))
				  col = cols[ncol/2-floor( (v/min)*(ncol/2))]
				  #col = "white"
				}
      }
				...
				else{#col = rgb(1, 0, 0, 0.1+0.9*(v/max))
