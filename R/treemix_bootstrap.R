  ...
  if(plotboot){
    if (is.null(boot.legend.location)){
      legend(max(d$x)*0.95,max(d$y), 
             ...
             pch = c(19,15,17, 4), 
             col=c(adjustcolor("green4", 0.97), adjustcolor("green2", 1),adjustcolor("olivedrab2", 1), adjustcolor("honeydew2", 1)), 
             cex = boot.legend.cex, title = "Bootstrap values", bty="n", border = F, box.lwd = 0) 
    }else{
