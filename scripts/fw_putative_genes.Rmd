...
```{r plot_lg8}
chrom<-"LG8"
nf <- layout(matrix(c(1,1,1,2,2,3), 2,3, byrow=TRUE), respect=TRUE)
...
rect(xleft=as.numeric(starts),xright=as.numeric(stops),
       ybottom=-0.04,ytop=1,col="indianred",border="indianred")
text(x=put.reg[put.reg$Chrom %in% chrom,"StartBP"],y=1.1,cex=0.6,
     labels=put.reg[put.reg$Chrom %in% chrom,"Gene"],
     srt=90,xpd=T)
#fav vs non fav
# fav.genes<-c("ARHGEF3","RHOGTP8","ATP6V0A1","TRIM14","TNS1","SLC26A3")
# starts<-as.numeric(put.reg[put.reg$Chrom %in% chrom & put.reg$Gene %in% fav.genes,"StartBP"])
# stops<-as.numeric(put.reg[put.reg$Chrom %in% chrom & put.reg$Gene %in% fav.genes,"StopBP"])
# rect(xleft=as.numeric(starts),xright=as.numeric(stops),
#        ybottom=-0.04,ytop=1,col="indianred",border="indianred")
# text(x=put.reg[put.reg$Chrom %in% chrom & put.reg$Gene %in% fav.genes,"StartBP"],y=1.1,cex=0.6,
#      labels=put.reg[put.reg$Chrom %in% chrom & put.reg$Gene %in% fav.genes,"Gene"],
#      srt=90,xpd=T)
# print(chrom)
# starts<-as.numeric(put.reg[put.reg$Chrom %in% chrom & !put.reg$Gene %in% fav.genes,"StartBP"])
# stops<-as.numeric(put.reg[put.reg$Chrom %in% chrom & !put.reg$Gene %in% fav.genes,"StopBP"])
# rect(xleft=as.numeric(starts),xright=as.numeric(stops),
#        ybottom=-0.04,ytop=1,col=alpha("indianred",0.5),border=alpha("indianred",0.5))
...
gene.out<-data.frame(BP=c(as.numeric(fst.gene),as.numeric(sal.gene),as.numeric(sdd.gene)),
  ...
  #add outlier loci
  clip(x1=min(as.numeric(pstart)),x2=max(as.numeric(pstop)),y1=0.99,y2=1.06)
  abline(v=fst.gene,col="black")
