...
*Overlap:
`r unique(put.reg[put.reg$rad.loci != "NA","Gene"])[unique(put.reg[put.reg$rad.loci != "NA","Gene"]) %in% unique(put.reg[put.reg$sdd.in != "NA","Gene"]) &unique(put.reg[put.reg$rad.loci != "NA","Gene"]) %in% unique(put.reg[put.reg$bfs.in != "NA","Gene"])&unique(put.reg[put.reg$rad.loci != "NA","Gene"]) %in% unique(put.reg[put.reg$nj.nb != "NA","Gene"])&unique(put.reg[put.reg$rad.loci != "NA","Gene"]) %in% unique(put.reg[put.reg$jostd.in != "NA","Gene"])]` 
...
legend(x=0.5,y=0.22,c(expression(Shared~italic(F)[ST]~Outliers),
                ...
                "Monophyletic FW Gene Tree",
                "FW Candidate Gene"),
       pch=c(124,25,124,124),lty=c(0,0,0,0),lwd=2,
       col=c("black","black","#08519c","indianred"),
...
plot(fst.dfs[[1]]$BP[fst.dfs[[1]]$Chr %in% chrom],
     fst.dfs[[1]]$Smoothed.AMOVA.Fst[fst.dfs[[1]]$Chr %in% chrom],
     type='n',ylim=c(0,1),bty="L",xlab="",ylab="",xaxt='n',yaxt='n',xlim=c(1000000,3500000))
axis(2,las=1,cex.axis=2)  
axis(1,cex.axis=2,at=c(1000000,3500000),labels=c(1,3.5))
#add putative gene
...
clip(x1=min(as.numeric(0)),x2=max(as.numeric(3500000)),y1=0.99,y2=1.06)
  ...
  lgnd<-c(lgnd,expression(delta~-divergence),"Jost's D","gene regions",
          expression("Shared"~italic(F[ST])~"outlier"),"Monophyletic Gene Tree")
  pchs<-c(rep(32,6),15,124,124)
  cols<-c(grp.colors[1],grp.colors[2],grp.colors[3],grp.colors[6],
          "#dfc27d","#a6611a","indianred","black","#08519c")
  ltys=c(1,1,2,1,1,1,0,0,0)
...
![LG 8 Figure](../fwsw_results/LG8.png)