...
Genes near monophyletic FW gene trees: `r unique(put.reg[put.reg$nj.nb != "NA","Gene"])`
Overlap:
...
```{r plotting_function}
gene.region.plot<-function(chrom,gene,put.reg,vcf,chrom.ld, fst.dfs,deltad=FALSE,
  ...
  smooth.fsts<-lapply(fst.dfs,function(df){
    ...
    this.smooth<-do.call("rbind",lapply(seq(1,nrow(this.df),(nrow(this.df)*0.15)/5),sliding.avg,
                                      dat=data.frame(Pos=this.df$BP,Fst=this.df$Corrected.AMOVA.Fst),
 ...
 # tmp.gene<- unlist(strsplit(gsub(paste(chrom,".",sep=""),"",pr.gene$bft.in[pr.gene$bft.in != "NA"]),","))
  ...
  if(is.data.frame(deltad)){
    points(dd.smooth$x,dd.smooth$y,col="cornflowerblue",type="l",lwd=2)
  }
  ...
  if(is.data.frame(deltad)){
    ...
    lgnd<-c(lgnd,
            expression(delta~-divergence),substitute(italic(g),list(g=gene)),"Outlier RAD loci")
  }else{
    lgnd<-unlist(lapply(names(fst.dfs),function(n) { substitute(paste(name,italic(F[ST]),sep=""),list(name=n)) }))
    lgnd<-c(lgnd,
                  substitute(italic(g),list(g=gene)),"Outlier RAD loci")
  }
  cols<-NULL
  ...
  legend("topleft",
         ...
         bty='n',lwd=c(lwds,2,0,0,0,0),lty=c(ltys,1,0,0,0,0),
         col=cols)
  }
  #add outlier loci
  ...
  abline(v=nb.gene,col=alpha("black",0.5))
  abline(v=nj.gene,col="#08519c")
}
```
...
tns1<-lapply(unique(put.reg[put.reg$Gene=="AQP3" & put.reg$Chrom %in% lgs,"Chrom"]),
...
arhgef3<-lapply(unique(put.reg[put.reg$Gene=="ARHGEF3" & put.reg$Chrom %in% lgs,"Chrom"]),
       gene.region.plot,gene="ARHGEF3",put.reg=put.reg,vcf=vcf,chrom.ld=chrom.ld,
       fst.dfs=fst.dfs,deltad=deltad,colors=colors,alphas=alphas,ltys=ltys)
dev.off()
...
par(mfrow=c(2,3),oma=c(2,2,2,2),mar=c(2,2,2,2))
nakatpase<-lapply(unique(put.reg[put.reg$Gene=="NAKATPase" & put.reg$Chrom %in% lgs,"Chrom"]),
       gene.region.plot,gene="NAKATPase",put.reg=put.reg,vcf=vcf,chrom.ld=chrom.ld,
       fst.dfs=fst.dfs,deltad=deltad,colors=colors,alphas=alphas,ltys=ltys)
dev.off()
...
ca2<-lapply(unique(put.reg[put.reg$Gene=="mucin" &put.reg$Chrom %in% lgs,"Chrom"]),
...
png("camkk1.png",height=5,width=10,units="in",res=300)
par(mfrow=c(1,2),oma=c(2,2,2,2),mar=c(2,2,2,2))
gene.region.plot("LG1","CAMKK1",put.reg,vcf,chrom.ld, fst.dfs,deltad=deltad,
                           colors=colors,alphas=alphas,ltys=ltys)
gene.region.plot("LG11","CAMKK1",put.reg,vcf,chrom.ld, fst.dfs,deltad=deltad,
...
par(mfrow=c(6,3),oma=c(2,2,2,2),mar=c(2,2,2,2))
rhogtp8<-lapply(unique(put.reg[put.reg$Gene=="RHOGTP8" & put.reg$Chrom %in% lgs,"Chrom"]),
       gene.region.plot,gene="RHOGTP8",put.reg=put.reg,vcf=vcf,chrom.ld=chrom.ld,
       fst.dfs=fst.dfs,deltad=deltad,colors=colors,alphas=alphas,ltys=ltys)
dev.off()
```

```{r making_Fig7, eval=FALSE,echo=FALSE}

png("Fig7candidateGenes.png",height=10,width=10,units="in",res=300)


par(mfrow=c(4,3),oma=c(2,2,4.5,2),mar=c(1,2,1,1))
gene.region.plot("LG1","mucin",put.reg,vcf,chrom.ld, fst.dfs,deltad=deltad,
                 colors=colors,alphas=alphas,ltys=ltys,legend=FALSE)
text(x=8500000,y=0.8,expression(italic(mucin)),xpd=T)
gene.region.plot("LG2","mucin",put.reg,vcf,chrom.ld, fst.dfs,deltad=deltad,
                 colors=colors,alphas=alphas,ltys=ltys,legend=FALSE)
text(x=1000000,y=0.8,expression(italic(mucin)),xpd=T)
gene.region.plot("LG8","mucin",put.reg,vcf,chrom.ld, fst.dfs,deltad=deltad,
                 colors=colors,alphas=alphas,ltys=ltys,legend=FALSE)
text(x=7100000,y=0.8,expression(italic(mucin)),xpd=T)
#row2
gene.region.plot("LG1","RHOGTP8",put.reg,vcf,chrom.ld, fst.dfs,deltad=deltad,
                 colors=colors,alphas=alphas,ltys=ltys,legend=FALSE)
text(x=5000000,y=0.8,expression(italic(RhoGTPase)),xpd=T)
gene.region.plot("LG5","RHOGTP8",put.reg,vcf,chrom.ld, fst.dfs,deltad=deltad,
                 colors=colors,alphas=alphas,ltys=ltys,legend=FALSE)
text(x=7500000,y=0.8,expression(italic(RhoGTPase)),xpd=T)
gene.region.plot("LG20","RHOGTP8",put.reg,vcf,chrom.ld, fst.dfs,deltad=deltad,
                 colors=colors,alphas=alphas,ltys=ltys,legend=FALSE)
text(x=2000000,y=0.8,expression(italic(RhoGTPase)),xpd=T)
#row3
gene.region.plot("LG1","TNS1",put.reg,vcf,chrom.ld, fst.dfs,deltad=deltad,
                 colors=colors,alphas=alphas,ltys=ltys,legend=FALSE)
text(x=17000000,y=0.8,expression(italic(TNS1)),xpd=T)
gene.region.plot("LG1","AQP3",put.reg,vcf,chrom.ld, fst.dfs,deltad=deltad,
                 colors=colors,alphas=alphas,ltys=ltys,legend=FALSE)
text(x=6500000,y=0.8,expression(italic(Aquaporin3)),xpd=T)
gene.region.plot("LG1","NAKATPase",put.reg,vcf,chrom.ld, fst.dfs,deltad=deltad,
                 colors=colors,alphas=alphas,ltys=ltys,legend=FALSE)
text(x=5500000,y=0.8,expression(italic(Na^'+'*K^'+'*ATPase)),xpd=T)
#row4
gene.region.plot("LG2","CAII",put.reg,vcf,chrom.ld, fst.dfs,deltad=deltad,
                 colors=colors,alphas=alphas,ltys=ltys,legend=FALSE)
text(x=4500000,y=0.8,expression(italic(CAII)),xpd=T)
gene.region.plot("LG2","CAMKK1",put.reg,vcf,chrom.ld, fst.dfs,deltad=deltad,
                 colors=colors,alphas=alphas,ltys=ltys,legend=FALSE)
text(x=6500000,y=0.8,expression(italic(CAMKK1)),xpd=T)
gene.region.plot("LG13","ARHGEF3",put.reg,vcf,chrom.ld, fst.dfs,deltad=deltad,
                 colors=colors,alphas=alphas,ltys=ltys,legend=FALSE)
text(x=1750000,y=0.8,expression(italic(ARHGEF3)),xpd=T)
