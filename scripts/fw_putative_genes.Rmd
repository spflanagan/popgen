...
put.reg<-do.call(rbind,apply(put.genes,1,function(gene){
  ...
  new<-data.frame(Gene=gene[[1]],Function=gene[[2]],Chromsome=gene[[3]],
                     Species=gene[[4]],Citation=gene[[5]],Scovelli_geneID=gene[[6]],Notes=gene[[7]],
                     Chrom=paste(as.character(gene.info[,1]),sep=",",collapse = ","),
                     StartBP=paste(as.numeric(as.character(gene.info[,2])),sep=",",collapse = ","),
                     StopBP=paste(as.numeric(as.character(gene.info[,3])),sep=",",collapse = ","))
  return(as.data.frame(new))
}))
