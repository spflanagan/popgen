...
#This is necessary for multiple steps in this process.
...
I'm running this one on abies using `nohup Rscript ~/Projects/popgen/scripts/dmc_model5.R > ~/Projects/popgen/dmc5.log 2>&1 &`. It's process 54368 (finished on Feb 19). Now 37092 on Mar 6
...
First, we randomize with respect to the reference allele. 
...
Combine files for models 4 and 5 that are separated by proposed selected site.
...
compLikelihood_mixed_svInd_all = lapply(1: length(selSite), function(i) {
  readRDS(paste("dmc/compLikelihood_mixed_svInd_p4LG8_selSite", i, ".RDS",
                sep = ""))
})
...
Running these on abies with nohup, process 39388 (6 March 2018).
