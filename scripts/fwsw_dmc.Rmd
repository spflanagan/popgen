...
allFreqs<-calc.allFreqs(vcf[vcf$`#CHROM` %in% c("LG6","LG9","LG10","LG11","LG13","LG14","LG16","LG18","LG20"),],
                   pop.list,pop.labs)
saveRDS(allFreqs,"dmc/neutralAlleleFreqs_p4LG8.RDS")
```
...
LG8<-readRDS("dmc/selectedRegionAlleleFreqs_p4LG8.RDS")
allFreqs<-readRDS("dmc/neutralAlleleFreqs_p4LG8.RDS")
...
I'm running this one on abies using `nohup Rscript ~/Projects/popgen/scripts/run_dmc.R >> ~/Projects/popgen/dmc_ne.log 2>&1 &`. Process 45048 on 9 March 2018.

## Estimate recombination rates

```{r rec_est,eval=FALSE}
ssc.map<-read.delim("~/Projects/SSC_linkage_map_20160701.txt",header=T)
...
write.table(map,"~/Projects/ssc_map.txt",sep=" ",row.names=FALSE, col.names = TRUE)
library(MareyMap)
startMareyMapGUI()
#using the GUI, I ran all three types of recombination rate estimators and saved the MapSet
```
```{r estimate_rec}
ssc.map<-read.delim("~/Projects/ssc_MareyMap.txt",comment.char = "#",sep=" ")
#genome-wide
r<-data.frame(Genome_wide=c(mean(ssc.map$slidingwindow,na.rm = TRUE)/1000000,
              ...
              LG8=c(mean(ssc.map$slidingwindow[ssc.map$map=="LG8"],na.rm = TRUE)/1000000,
                            mean(ssc.map$spline[ssc.map$map=="LG8"],na.rm = TRUE)/1000000,
                            mean(ssc.map$loess[ssc.map$map=="LG8"],na.rm = TRUE)/1000000),
              LG8_first4Mb=c(mean(ssc.map$slidingwindow[ssc.map$map=="LG8" & ssc.map$phys<4*10^6],na.rm = TRUE)/1000000,
                            mean(ssc.map$spline[ssc.map$map=="LG8"& ssc.map$phys<4*10^6],na.rm = TRUE)/1000000,
                            mean(ssc.map$loess[ssc.map$map=="LG8"& ssc.map$phys<4*10^6],na.rm = TRUE)/1000000),
...
#plot LG8
plot(set[["LG8"]]@interpolations$slidingwindow@physicalPositions,
     set[["LG8"]]@interpolations$slidingwindow@rates,type="l",
     ylim=c(-5,25),xlab="",ylab="",xaxt='n',lwd=2)
abline(h=0,lty=2)
axis(1,at=seq(0,1*10^7,2*10^6),labels = seq(0,10,2))
mtext("Position on LG8 (Mb)",1,line=2)
mtext("Recombination Rate (cM/Mb)",2,line=2)
lines(set[["LG8"]]@interpolations$spline@physicalPositions,
      set[["LG8"]]@interpolations$spline@rates,type="l",col="blue",lwd=2)
lines(set[["LG8"]]@interpolations$loess@physicalPositions,
      set[["LG8"]]@interpolations$loess@rates,type="l",lwd=2,col="forestgreen")
...
patterns<-c("p4LG8_8300_2_8","p4LG8_83000_2_8","p4LG8_830000_2_8","p4LG8_8300000_2_8",
            "p4LG8_8300_2_7","p4LG8_83000_2_7","p4LG8_830000_2_7","p4LG8_8300000_2_7",
            "p4LG8_8300_6_6","p4LG8_83000_6_6","p4LG8_830000_6_6","p4LG8_8300000_6_6",
            "p4LG8_8300_5_6","p4LG8_83000_5_6","p4LG8_830000_5_6","p4LG8_8300000_5_6")
par(mfrow=c(4,4),mar=c(1.5,1.5,0.5,0.5),oma=c(2,2,2,2))
ne.complikes<-lapply(patterns,plot.complike,leg=FALSE,lab=FALSE)
mtext("Position on LG8",1,outer=TRUE,cex=0.8,line=.5)
...
ne.mcles<-lapply(ne.complikes, function(max.complikes){
  ...
  Ne<-as.numeric(gsub("p4LG8_(\\d+)_.*","\\1",pattern))
  r<-gsub("p4LG8_\\d+_(\\d_\\d)","\\1",pattern)
  rec<-rs[r]
...
max.complikes<-plot.complike("p4LG8_10000r2",leg=FALSE)
mcle<-get.mcle(max.complikes[[1]],Ne=5000,rec=2*10^-7)
kable(mcle)
```
