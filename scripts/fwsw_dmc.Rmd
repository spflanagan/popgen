...
LG4<-calc.allFreqs(vcf[vcf$`#CHROM` == "LG4",],pop.list,pop.labs)
...
r<-data.frame(Genome_wide=c(mean(ssc.map$slidingwindow,na.rm = TRUE)/1000000,
              ...
              LG4_first4Mb=c(mean(ssc.map$slidingwindow[ssc.map$map=="LG4" & ssc.map$phys<4*10^6],na.rm = TRUE)/1000000,
                            mean(ssc.map$spline[ssc.map$map=="LG4"& ssc.map$phys<4*10^6],na.rm = TRUE)/1000000,
                            mean(ssc.map$loess[ssc.map$map=="LG4"& ssc.map$phys<4*10^6],na.rm = TRUE)/1000000),
              row.names = c("Sliding Window","Spline","Loess"))
kable(r,caption="Recombination rate estimates")
```
```{r plot_recLG4,eval=FALSE}
...
```{r plot_chosen,eval=FALSE}
selSite = positions[seq(1, length(positions[positions<10000000]), length.out = 100)]
max.complikes<-plot.complike("p4LG4_10000r2",leg=FALSE)
