...
I'm running this one on abies using `nohup Rscript ~/Projects/popgen/scripts/run_dmc.R >> ~/Projects/popgen/dmc_ne.log 2>&1 &`. Process 30566 on 8 March 2018.
...
plot.complike<-function(pattern){
...
```{r plot_ne}
par(mfrow=c(2,2),mar=c(4,3,0.5,0.5),oma=c(2,2,2,2))
patterns<-c("p4LG8_8300","p4LG8_83000","p4LG8_830000","p4LG8_8300000")
ne.complikes<-lapply(patterns,plot.complike)
```
### Get maximum composite likelihood estimates

```{r get_mcle_fxn}
#These won't work if the parameters aren't specified as above
get.mcle<-function(max.complikes,Ne,selSite= positions[seq(1, length(positions), length.out = 50)],
  ...
  ## Model 1
  m1<-getMCLEind(max.complikes[grep("ind",names(max.complikes))], selSite, sels)
  kable(m1,caption="model1")
  ## Model 2
  m2<-getMCLEmig(max.complikes[grep("mig\\d+",names(max.complikes))], selSite, sels, migs, sources)
  kable(m2,caption="model2")
  ## Model 3
  ...
  kable(rbind(m3,m4,m5),caption="models 3, 4, and 5")
...
ne.mcles<-lapply(ne.complikes, function(max.complikes){
  ...
  Ne<-as.numeric(gsub(".*_(\\d+)","\\1",pattern))
  max.complikes<-max.complikes[[1]]
  get.mcle(max.complikes,Ne)
})
```
