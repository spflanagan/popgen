...
adj.coor<-as.data.frame(cbind(site=as.character(mar.coor$site), 
	...
	lat.h=as.numeric(mar.coor$lat+0.25), 
	lon.l=as.numeric(mar.coor$lon-0.25),
	lon.r=as.numeric(mar.coor$lon),
	lon.h=as.numeric(mar.coor$lon+0.25)), stringsAsFactors = FALSE)
...
#	wod.temp<-wod.filt[round(wod.filt$Long,1)==adj.coor$lon.r[i],]
