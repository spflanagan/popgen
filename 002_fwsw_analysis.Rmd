...
```{r StacksFsts}
```
```{r compare2ScovelliGenome,eval=TRUE}
```
```{r read_compare}
...
```
```{r PlotDeltaD,results="hide",eval=FALSE}
```
...
*I would like to update this - 22 Mar 2018*
...
#smooth the statistics
dd.smooth<-smooth.per.chrom(dd,lgs,df.chrom="Chrom",df.bp="plot.pos",df.stat="deltad",out.name="dd",span=0.1,degree=2) 
pi.smooth<-smooth.per.chrom(pi,lgs,df.chrom="Chrom",df.bp="plot.pos",df.stat="Pi",out.name="pi",span=0.1,degree=2) 
ht.smooth<-smooth.per.chrom(ht,lgs,df.chrom="Chrom",df.bp="plot.pos",df.stat="Het",out.name="ht",span=0.1,degree=2) 
jd.smooth<-smooth.per.chrom(jd,lgs,df.chrom="Chrom",df.bp="plot.pos",df.stat="D",out.name="jd",span=0.1,degree=2) 

```
```{r plot_smoothStats}
comp.col<-c(Het="#80cdc1",pi="#018571",Fst="black",D="#a6611a",deltad="#dfc27d")
plot.smooth.stats<-function(df,smooth,color,ylabel,...){
  dd<-fst.plot(fst.dat = df,...)
  mtext(ylabel,2,line=2,cex=0.75)
  ...
  points(smooth[[1]]$plot.pos[smooth[[1]]$direction=="upper"],smooth[[1]]$dd[smooth[[1]]$direction=="upper"],
         pch=24,bg=color,col=color,cex=0.5) #add  outliers
  points(smooth[[1]]$plot.pos[smooth[[1]]$direction=="lower"],smooth[[1]]$dd[smooth[[1]]$direction=="lower"],
...
t<-plot.smooth.stats(df=dd,smooth=dd.smooth,color=comp.col["deltad"],fst.name="deltad",bp.name="Pos",
...
xlab.lgs(dd,lgs,lgn,"Chrom","plot.pos",adj=1, xpd=TRUE,y=-0.05,cex=0.75)
#plot pi
p<-plot.smooth.stats(df=pi,smooth=pi.smooth,color=comp.col["pi"],fst.name="Pi",chrom.name = "Chrom",bp.name="Pos",
...
#plot heterozygosity
h<-plot.smooth.stats(df=ht,smooth=ht.smooth,color=comp.col["Het"],fst.name="Het",chrom.name = "Chrom",bp.name="Pos",
                  ylabel=expression(italic(H)),
                  axis=0.75,scaffold.widths=scaff.starts,pch=19,scaffs.to.plot = scaffs)
xlab.lgs(ht,lgs,lgn,"Chrom","plot.pos",adj=1, xpd=TRUE,y=-0.05,cex=0.75)
#plot Jost's D
d<-plot.smooth.stats(df=jd,smooth=jd.smooth,color=comp.col["D"],fst.name="D",chrom.name = "Chr",bp.name="POS",
                  ylabel=expression("Jost's"~italic(D)),
                  axis=0.75,scaffold.widths=scaff.starts,pch=19,scaffs.to.plot = scaffs)
xlab.lgs(jd,lgs,lgn,"Chr","plot.pos",adj=1, xpd=TRUE,y=-0.05,cex=0.75)
...
```{r dd_compare}
#in the gene
sdd.in<-outlier.in.region(dd.smooth[[2]],put.reg,oBP = "Pos")
...
```{r dd_nearby}
#in the gene
sdd.nb<-outlier.nearby(dd.smooth[[2]],put.reg,chrom.ld,oBP = "Pos")
```
...
```{r josts_d}
jost.in<-outlier.in.region(jd.smooth[[2]],put.reg,oChrom="chr",oBP="pos")
jost.nb<-outlier.nearby(jd.smooth[[2]],put.reg,chrom.ld,oChrom = "chr",oBP="pos")
...
bal<-balancing.sel(pi.smooth[[1]],"plot.pos",ht.smooth[[1]],"plot.pos",
                   dd.smooth[[1]],"plot.pos",jd.smooth[[1]],"plot.pos")
...
```{r get_bp_smooth}
#smooth the statistics
dd.smooth<-smooth.per.chrom(dd,lgs,df.chrom="Chrom",df.bp="Pos",df.stat="deltad",out.name="dd.bp",span=0.1,degree=2) 
pi.smooth<-smooth.per.chrom(pi,lgs,df.chrom="Chrom",df.bp="Pos",df.stat="Pi",out.name="pi.bp",span=0.1,degree=2) 
ht.smooth<-smooth.per.chrom(ht,lgs,df.chrom="Chrom",df.bp="Pos",df.stat="Het",out.name="ht.bp",span=0.1,degree=2) 
jd.smooth<-smooth.per.chrom(jd,lgs,df.chrom="Chrom",df.bp="Pos",df.stat="D",out.name="jd.bp",span=0.1,degree=2) 
...
fav.genes<-c("ARHGEF3", "ATP6V0A1", "ATP6V1A", "FLT4", "OSBPL8", "RHOGTP8", "SLC2A13", "SPG1", "STAT3", "TIMP3", "VATPase",
             "IGF-I","TAAR","DCT")
...
png("ARGHEF3.png",height=5,width=10,units="in",res=300)       
...
outside.legend("top",legend=c(leg.text,expression(italic(ARHGEF3))),pch=leg.pchs,lty=leg.ltys,lwd=2,
               bty='n',ncol=5,cex=1.25,x.intersp=0,col=leg.cols)
dev.off()

png("ATP6V0A1.png",height=5,width=10,units="in",res=300)       
par(mfrow=c(1,2),oma=c(2,2,2,2),mar=c(2,2,2,2))
atp6v0a1<-lapply(unique(put.reg[put.reg$Gene=="ATP6V0A1" & !is.na(put.reg$rad.loci),"Chrom"]),
       gene.region.plot,gene="ATP6V0A1",put.reg=put.reg,vcf=vcf,chrom.ld=chrom.ld,
       fst.dfs=fst.dfs,deltad=deltad,D=jostd,colors=colors,alphas=alphas,ltys=ltys,
       smooth=FALSE,fst.name="Smoothed.AMOVA.Fst",txt.cex=1.3,bases="MB",legend=FALSE)
outside.legend("top",legend=c(leg.text,expression(italic(ATP6VOA1))),pch=leg.pchs,lty=leg.ltys,lwd=2,
               bty='n',ncol=5,cex=1.25,x.intersp=0,col=leg.cols)
dev.off()

png("ATP6V1A.png",height=5,width=10,units="in",res=300)       
par(mfrow=c(1,2),oma=c(2,2,2,2),mar=c(2,2,2,2))
atp6v1a<-lapply(unique(put.reg[put.reg$Gene=="ATP6V1A" & !is.na(put.reg$rad.loci),"Chrom"]),
       gene.region.plot,gene="ATP6V1A",put.reg=put.reg,vcf=vcf,chrom.ld=chrom.ld,
       fst.dfs=fst.dfs,deltad=deltad,D=jostd,colors=colors,alphas=alphas,ltys=ltys,
       smooth=FALSE,fst.name="Smoothed.AMOVA.Fst",txt.cex=1.3,bases="MB",legend=FALSE)
outside.legend("top",legend=c(leg.text,expression(italic(ATP6V1A))),pch=leg.pchs,lty=leg.ltys,lwd=2,
               bty='n',ncol=5,cex=1.25,x.intersp=0,col=leg.cols)
dev.off()

png("FLT4.png",height=4,width=6,units="in",res=300)       
par(mfrow=c(1,1),oma=c(2,2,2,2),mar=c(2,2,2,2))
flt4<-lapply(unique(put.reg[put.reg$Gene=="FLT4" & !is.na(put.reg$rad.loci),"Chrom"]),
       gene.region.plot,gene="FLT4",put.reg=put.reg,vcf=vcf,chrom.ld=chrom.ld,
       fst.dfs=fst.dfs,deltad=deltad,colors=colors,alphas=alphas,ltys=ltys,
       smooth=FALSE,fst.name="Smoothed.AMOVA.Fst",bases="MB")
dev.off()

png("OSBPL8.png",height=4,width=6,units="in",res=300)  
par(mfrow=c(1,1),oma=c(2,2,2,2),mar=c(2,2,2,2))
osbpl8<-lapply(unique(put.reg[put.reg$Gene=="OSBPL8" & !is.na(put.reg$rad.loci),"Chrom"]),
       gene.region.plot,gene="OSBPL8",put.reg=put.reg,vcf=vcf,chrom.ld=chrom.ld,
       fst.dfs=fst.dfs,deltad=deltad,colors=colors,alphas=alphas,ltys=ltys,
       smooth=FALSE,fst.name="Smoothed.AMOVA.Fst",bases="MB")
dev.off()

png("RHOGTP8.png",height=5,width=10,units="in",res=300)  
par(mfrow=c(2,5),oma=c(2,2,2,2),mar=c(2,2,2,2))
rhogtp8<-lapply(unique(put.reg[put.reg$Gene=="RHOGTP8" & !is.na(put.reg$rad.loci),"Chrom"]),
       ...
       fst.dfs=fst.dfs,deltad=deltad,D=jostd,colors=colors,alphas=alphas,ltys=ltys,
       smooth=FALSE,fst.name="Smoothed.AMOVA.Fst",bases="MB",legend=FALSE)
outside.legend("top",legend=c(leg.text,expression(italic(RHOGTP))),pch=leg.pchs,lty=leg.ltys,lwd=2,
               bty='n',ncol=5,cex=1.25,x.intersp=0,col=leg.cols)
dev.off()

png("SLC2A13.png",height=4,width=6,units="in",res=300)  
par(mfrow=c(2,2),oma=c(2,2,1,2),mar=c(2,2,1,2))
slc2a13<-lapply(unique(put.reg[put.reg$Gene=="SLC2A13" & !is.na(put.reg$rad.loci) & put.reg$Chrom %in% lgs,"Chrom"]),
       gene.region.plot,gene="SLC2A13",put.reg=put.reg,vcf=vcf,chrom.ld=chrom.ld,
       fst.dfs=fst.dfs,deltad=deltad,D=jostd,colors=colors,alphas=alphas,ltys=ltys,
       smooth=FALSE,fst.name="Smoothed.AMOVA.Fst",bases="MB",legend=FALSE) 
outside.legend(x=0.25,y=0.05,legend=c(leg.text,expression(italic(SLC2A13))),pch=leg.pchs,lty=leg.ltys,lwd=2,
               bty='n',ncol=1,cex=1,x.intersp=0,col=leg.cols)
dev.off()

png("SPG1.png",height=4,width=6,units="in",res=300)  
par(mfrow=c(1,1),oma=c(1,2,2,2),mar=c(2,2,2,2))
spg1<-lapply(unique(put.reg[put.reg$Gene=="SPG1" & !is.na(put.reg$rad.loci),"Chrom"]),
       gene.region.plot,gene="SPG1",put.reg=put.reg,vcf=vcf,chrom.ld=chrom.ld,
       fst.dfs=fst.dfs,deltad=deltad,D=jostd,colors=colors,alphas=alphas,ltys=ltys,
       smooth=FALSE,fst.name="Smoothed.AMOVA.Fst",bases="MB",legend=FALSE) 
outside.legend("top",legend=c(leg.text,expression(italic(SPG1))),pch=leg.pchs,lty=leg.ltys,lwd=2,
               bty='n',ncol=3,cex=0.75,x.intersp=0,col=leg.cols)
dev.off()


png("STAT3.png",height=4,width=6,units="in",res=300)  
par(mfrow=c(1,1),oma=c(2,2,2,2),mar=c(2,2,2,2))
stat3<-lapply(unique(put.reg[put.reg$Gene=="STAT3" & !is.na(put.reg$rad.loci),"Chrom"]),
       gene.region.plot,gene="STAT3",put.reg=put.reg,vcf=vcf,chrom.ld=chrom.ld,
       fst.dfs=fst.dfs,deltad=deltad,D=jostd,colors=colors,alphas=alphas,ltys=ltys,
       smooth=FALSE,fst.name="Smoothed.AMOVA.Fst",bases="MB",legend=FALSE) 
outside.legend("top",legend=c(leg.text,expression(italic(STAT3))),pch=leg.pchs,lty=leg.ltys,lwd=2,
               bty='n',ncol=3,cex=0.75,x.intersp=0,col=leg.cols)
dev.off()

png("TIMP3.png",height=4,width=6,units="in",res=300)  
par(mfrow=c(1,1),oma=c(2,2,1,1),mar=c(2,2,2,2))
timp3<-lapply(unique(put.reg[put.reg$Gene=="TIMP3" & !is.na(put.reg$rad.loci),"Chrom"]),
       gene.region.plot,gene="TIMP3",put.reg=put.reg,vcf=vcf,chrom.ld=chrom.ld,
       fst.dfs=fst.dfs,deltad=deltad,D=jostd,colors=colors,alphas=alphas,ltys=ltys,
       smooth=FALSE,fst.name="Smoothed.AMOVA.Fst",bases="MB")
dev.off()

png("VATPase.png",height=5,width=8,units="in",res=300) 
...
vatpase<-lapply(unique(put.reg[put.reg$Gene=="VATPase" & !is.na(put.reg$rad.loci),"Chrom"]),
       ...
       fst.dfs=fst.dfs,deltad=deltad,D=jostd,colors=colors,alphas=alphas,ltys=ltys,
       smooth=FALSE,fst.name="Smoothed.AMOVA.Fst",txt.cex=1.3,bases="MB",legend=FALSE)
outside.legend("top",legend=c(leg.text,expression(italic(VATPase))),pch=leg.pchs,lty=leg.ltys,lwd=2,
               bty='n',ncol=3,cex=1,x.intersp=0,col=leg.cols)
dev.off()
```

![](../fwsw_results/ARGHEF3.png)
![](../fwsw_results/ATP6V0A1.png)
![](../fwsw_results/ATP5V1A.png)
![](../fwsw_results/FLT4.png)
![](../fwsw_results/OSBPL8.png)
![](../fwsw_results/RHOGTP8.png)
![](../fwsw_results/SLC2A13.png)
![](../fwsw_results/SPG1.png)
![](../fwsw_results/STAT3.png)
![](../fwsw_results/TIMP3.png)
![](../fwsw_results/VATPase.png)
...
plot(fst.dfs[[1]]$BP[fst.dfs[[1]]$Chr %in% chrom],
  ...
  #Pi
  points(pi.smooth[[2]][pi.smooth[[2]][,"chr"]%in% chrom,c("pos","smoothed.stats")],
       type="l",lwd=2,col=comp.col["pi"])
  upp.low.pts(smooth=pi.smooth,chrom=chrom,color=comp.col["pi"],stat="Pi",pos.name="Pos")
  #Het
  points(ht.smooth[[2]][ht.smooth[[2]][,"chr"]%in% chrom,c("pos","smoothed.stats")],
         type="l",col=comp.col["Het"],lwd=2)
  upp.low.pts(smooth=ht.smooth,chrom=chrom,color=comp.col["Het"],stat="Het",pos.name="Pos")
  
  #deltad
  points(dd.smooth[[2]][dd.smooth[[2]][,"chr"]%in% chrom,c("pos","smoothed.stats")],
         type="l",col=comp.col["deltad"],lwd=2)
  upp.low.pts(smooth=dd.smooth,chrom=chrom,color=comp.col["deltad"],stat="deltad",pos.name="Pos")

  #Josts D
  points(jd.smooth[[2]][jd.smooth[[2]][,"chr"]%in% chrom,c("pos","smoothed.stats")],
         type="l",col=comp.col["D"],lwd=2)
  upp.low.pts(smooth=jd.smooth,chrom=chrom,color=comp.col["D"],stat="D",pos.name="Pos")
