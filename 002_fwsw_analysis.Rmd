...
sweed<-do.call(rbind,lapply(sweed.files,function(file){
  ...
  if(grep("lg",file)){ #correct lowercase chrom names
    swlg$Chrom<-paste("LG",gsub(".*lg(\\d+)$","\\1",file),sep="")
  }else if(grep("LG",file)){
...
sweed<-assign.plotpos(sweed,plot.scaffs,bounds,df.bp="Position")
plot(c(0,max(sweed$plot.pos)),c(0,max(sweed$Likelihood)+1),bty="L",type = 'n',xlab="Position on Chromosome",
     ylab="SweeD Likelihood",xaxt='n',las=1)
for(i in 1:length(lgs)){
  if(nrow(sweed[sweed$Chrom %in% lgs[i],])>0){
    if(i %% 2 == 0){ this.col<-"lightgrey" } else { this.col <- "darkgrey" }
    points(sweed$plot.pos[sweed$Chrom==lgs[i]],sweed$Likelihood[sweed$Chrom==lgs[i]],pch=19,
           col=this.col)
  }
}
#or try this
sweed.plot<-fst.plot(sweed,fst.name = "Likelihood", bp.name = "Position",chrom.name = "Chrom", 
                    ...
                    pt.cols=c("lightgrey","darkgrey"),pt.cex=1,axis.size=1)
```
...
```{r plot_putative_regions,eval=TRUE}
...
plot.complike<-function(pattern,leg=TRUE,lab=TRUE,plot_range = NA){
  max.likes<-calc.max.complike(pattern)
  
  if(is.na(plot_range)){ plot_range = range(max.likes) }
...
max.complikes.sel<-plot.complike("p4LG4_100000_2_9_forReal",
                                 plot_range=range(fst.dfs[[1]]$BP[fst.dfs[[1]]$Chr %in% chrom]))
