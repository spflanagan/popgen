...
smooth.per.chrom<-function(df,lgs,df.chrom,df.bp,...){#span=0.1,degree=2
  smooth.low<-data.frame() #lower outliers
  smooth.up<-data.frame() #upper outliers
  for(i in 1:length(lgs)){
    this.chrom<-df[df[df.chrom] %in% lgs[i],]
    this.smooth<-loess.smooth(this.chrom[df.bp],this.chrom[df.chrom],...) 
...
```{r josts_d}
#find outliers
jost.out<-jostd[jostd$D>=quantile(jostd$D,0.99) | jostd$D<=quantile(jostd$D,0.01),]
...
xlims<-lapply(chroms2plot,function(lg,ld,genes,vcf){
  xs<-c(min(genes[genes$Chr == lg,"plot.min"]-ld[lg]),
        max(genes[genes$Chr == lg,"plot.max"]+ld[lg]))
  if(xs[1] < 0){
    xs[1]<-0
  }
