...
Now I need to re-make the figure...

```{r dapcData}
```
```{r rereadFiles, echo=FALSE}
...
## SETUP
comp.col<-c(Het="#80cdc1",pi="#018571",Fst="black",D="#a6611a",deltad="#dfc27d")
i<-2 #LG8 is the second in the 'chroms2plot' list
...
regmin<-pos-mean(ld$pos.diff[ld$Chrom=="LG8" & ld$LocIDA==pos & ld$D. > 0.5])
regmax<-pos+mean(ld$pos.diff[ld$Chrom=="LG8" & ld$LocIDA==pos & ld$D. > 0.5])

# Find the putative gene regions
g<-put.reg[put.reg$Chrom == "LG8",c("Gene","Chrom","StartBP","StopBP")]
#remove duplicates
g<-g[g$Gene !="ATP6V1A",] 
...
```{r}
## PLOT
png("LG8_balancingsel.png",height=6,width=10,units="in",res=300)
