...
```
```{r id_monophyletic}
ftsnp.mono<-ftrees.snp[ftrees.snp$FWMonophyletic == TRUE,]
fthap.mono<-ftrees.hap[ftrees.hap$FWMonophyletic == TRUE,]
```

There are `r nrow(ftrees.snp)` monophyletic SNP trees and `r nrow(ftrees.hap)` haplotype trees. The SNP data had `r nrow(ftrees.snp[is.na(ftrees.snp$FWMonophyletic),])` loci with data missing from at least population; thes were excluded. The haplotype data had `r nrow(ftrees.hap[is.na(ftrees.hap$FWMonophyletic),])` loci with data missing from at least one population, which were excluded. The SNP dataset has `r nrow(ftsnp.mono)` monophyletic SNPs and the haplotype dataset has `r nrow(fthap.mono)` monophyletic haplotypes.
...
```{r}
ftmono<-ftsnp.mono
fig5.name<-"p4_stacks_fsts_nj_fwsw_bf.png"
...
![Figure 5. Fsts](fwsw_results/p4_stacks_fsts_nj_fwsw_bf.png)
...
jostd<-read.delim("p4.jostd.perlocus.txt",header=FALSE)
```
