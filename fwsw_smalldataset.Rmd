...
1. one generated from all pairwise comparisons of populations, containing SNPs only found in all 16 populations, in 75% of individuals, and with a minor allele frequency of at least 5%.
2. one generated from comparing lumped 'freshwater' and 'saltwater' populations, containing SNPs found in 50% of individuals and with a minor allele frequency of at least 5%.

I did the majority of the analyses using set #2, but would like to explore what changes if I use the other dataset.
...
read_chunk("../scripts/fwsw_analysis.R")
```
```{r fwsw_analysis_setup, include=FALSE}
```
```{r read_vcf}
...
```{r AFS,echo=FALSE}
```
![AFS_with _separateSNPs]("All_AFS.png")
