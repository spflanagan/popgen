...
```{r TreemixSetup,eval=FALSE}
```
```{r FLPBoutgroup, results="hide",fig.keep="high",eval=FALSE}
```
...
```{r FLPBmigration, results="hide",fig.keep="high",eval=FALSE}
```

Look at the p-values: do migration events always improve the fit of the data?

```{r FLPBpvals,eval=FALSE}
```
...
```
```{r TreemixFavorite, echo=FALSE,results="hide",fig.keep="high",eval=FALSE}
```
...
```
```{r HeatmapCols,eval=FALSE}
```
```{r treemix,eval=FALSE}
```
```{r Fig3Setup,eval=FALSE}
```
...
```{r calcAFS,eval=TRUE,results="hide"}
```
```{r plotAFS, echo=FALSE,eval=TRUE}
```
