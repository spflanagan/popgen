...
I ran the optimize functions from (dportik's dadi pipeline)[https://github.com/dportik/dadi_pipeline] for each of the 7 populations. Now I need to see what the optimal parameters are.

```{r}
lapply(pop.list, function(pop,dadi.dir="dadi_analysis"){
  if(length(grep("/^",dadi.dir))==0){
    dadi.dir<-paste(dadi.dir,"/",sep="")
  }
  path<-paste(dadi.dir,pop,sep="")
  opt.files<-list.files(path = path,pattern = "optimized.txt",full.names = TRUE)
  opts<-lapply(opt.files,function(opt.file){
    model.opt<-read.delim(opt.file,sep='\t',header=TRUE)
    model.opt<-model.opt[-which(model.opt$Model=="Model"),]
    model.opt[which.min(as.numeric(as.character(model.opt$AIC))),]
  })
})
```

Once I have that, then I'll run the optimal set and do model comparison to choose the most likely single-population scenarios.
