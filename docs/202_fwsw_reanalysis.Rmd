...
filename<-read.pcadapt("stacks/populations_subset75/batch_2.plink.ped",type="ped") #need to remove the first line with a # 
x<-pcadapt(filename, K=20)
plot(x,option="screeplot")
```

*K=4* seems like the best choice here.
...
We need to choose a cutoff for outlier detection. I'll start with the qvalue approach.
```{r}
library(qvalue)
qval <- qvalue(pa$pvalues)$qvalues
alpha <- 0.1
outliers <- which(qval < alpha)
length(outliers)
snp_pc<-get.pc(pa,outliers)
```

What about LD thinning appraoch? They recommend displaying the loadings and evaluate if loadings are clustered in single or several genomic regions
...
This suggests that loadings are not clustered (assuming these are grouped by space). Now let's look chromosome by chromosome:

```{r}
map<-read.delim("stacks/populations_subset75/batch_2.plink.map",header = FALSE,comment.char = "#")
par(mfrow=c(6,4),mar=c(3,3,2,1.5))
l<-lapply(lgs, function(lg,map){
  plot(res$loadings[which(map$V1 %in% lg), 1], pch = 19, cex = .3, xlab = paste0("Position on ", lg), ylab = "Loadings PC 1")
  mtext(lg,3,outer=FALSE)
},map=map)
```

None of the LGs seem to have huge clusters of outliers.

#### Look for overlap

To do this, we can use the map information but we need to slightly modify the locus ID

```{r}
map$LocID<-gsub("(\\d+)_\\d+","\\1",map$V2)
snp_pc$ID<-map$LocID[outliers]
all_shared<-snp_pc$ID[snp_pc$ID %in% as.character(all.shared)]
```

There are `r length(unique(all_shared))` RAD loci with shared outliers in all of the Stacks analyses and in the PCAdapt outlier analysis. These RAD loci are located on chromosomes `r unique(map[map$LocID %in% all_shared,"V1"])`. This honestly isn't all that much overlap -- but the ones on LG12 are all right next to each other. 

### Bayenv?

I think I need to run it again - but here's what I did previously.
