...
Let's plot all of the 2-edge trees from the multiple replicates

```{r treemixBootstrapM2s,message=FALSE,results='hide',fig.show='true',fig.width=8,fig.height=10,fig.cap="Plot of the tree with no migration edge, the tree with 2 migration edges that has been analyzed, and all of the bootstrap replicates with two migration edges."}
par(mfrow=c(3,4),mar=c(1,1,1,2),oma=c(1,1,1,2),xpd=TRUE)
t0<-plot_tree("treemix/fwsw_k100bFLPBr",scale=T,mbar=F,cex = 1.5,
              lwd=2,mig_left=FALSE,disp=0.0002,scadj=0.05)
t2<-plot_tree("treemix/fwsw_k100bFLPBrm2",scale=T,mbar=T,cex = 1.5,
              lwd=2,mig_left=FALSE,disp=0.0002,scadj=0.05)
for(i in 1:10){
  t2<-plot_tree(paste0("treemix/fwsw_k100bFLPBrm2_",i),
                scale=T,mbar=T,cex = 1.5,
              lwd=2,mig_left=FALSE,disp=0.0002,scadj=0.05)
}
```
...
fw_SNPinfo<-merge(fw_SNPinfo,
                  bayenv_dat[,c("index","XtX","logSalBF","logTemBF","logSegBF")],
                  by.x="ID",by.y="index",all.x=TRUE)
saveRDS(fw_SNPinfo,"fw_SNPinfo.RDS")
```
...
```{r rearrangePutatives,eval=TRUE}
