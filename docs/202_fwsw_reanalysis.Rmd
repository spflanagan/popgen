...
fst_dat<-list(upd.st,upd.sa,upd.sf)
fsts<-plot_multiple_LGs(list_fsts = fst_dat,fst_name = "Corrected.AMOVA.Fst",bp_name="BP",chr_name="Chr",
...
tx.sig<-upd.st[upd.st$Fisher.s.P<0.01,"Locus.ID"]
al.sig<-upd.sa[upd.sa$Fisher.s.P<0.01,"Locus.ID"]
fl.sig<-upd.sf[upd.sf$Fisher.s.P<0.01,"Locus.ID"]
all.shared<-fl.sig[fl.sig %in% la.sig & fl.sig %in% al.sig & fl.sig %in% tx.sig]
```
