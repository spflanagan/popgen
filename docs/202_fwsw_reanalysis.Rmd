...
To choose the optimal number of migration edges, we used the R package optM [@fitakOptMPackageOptimize2019].
...
```
```{r PcadaptK4-3, fig.cap="Distribution of the  for the PCAdapt outlier analysis with the subsetted dataset and 4 PC axes retained in the analysis.",eval=FALSE}
plot(res, option="stat.distribution",xlim=c(0,100))
```
...
```{r Pcadapt2Snpinfo, eval=TRUE}
fw_SNPinfo<-readRDS("fw_SNPinfo.RDS")
fw_SNPinfo$pcadaptQ<-qval
fw_SNPinfo$pcadaptPC<-get.pc(res,1:length(qval))$PC
fw_SNPinfo$pcadaptOut<-FALSE
fw_SNPinfo$pcadaptOut[outliers]<-TRUE
saveRDS(fw_SNPinfo,"fw_SNPinfo.RDS")
```

I should note that for some of these PCAdapt gives "NA" because it has its own built-in pruning, including a minor allele frequency setting, which apparently works somewhat differently from Stacks' pruning. 
...
There are `r length(bothXtX)` SNPs that are XtX outliers in both Bayenv analyses and `r length(bothSal)` salinity-associated outliers in both.

```{r bayenvNoFL2fw_SNPinfo}
fw_SNPinfo<-merge(fw_SNPinfo,
                  bayenv_noFL[,c("ID","XtX_noFL","logSalBF_noFL","logTemBF_noFL","logSegBF_noFL")],
                  by.x="ID",by.y="ID",all.x=TRUE)
saveRDS(fw_SNPinfo,"fw_SNPinfo_noFL.RDS")
```
...
We can create upset plots to show overlap between pairwise Stacks comparisons and between all outlier methods.
...
```{r CreateOutliersImage, eval=TRUE}
library(magick);library(multipanelfigure)
