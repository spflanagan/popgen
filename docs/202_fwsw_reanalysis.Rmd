...
There are `r length(unique(all.shared))` outliers (as determined by Fisher's P from stacks < 0.01). But because of the large pairwise 
...
Before running the analysis, I need to subset the environmental data too.
```{r extractFLenv, eval=FALSE}
env_dat<-read.csv("bayenv/env_data_sub75.csv",row.names=1)
clust<-read.delim("bayenv/noFL/sub75.noFL.clust",header=FALSE,sep=" ")
env_keep<-env_dat[,colnames(env_dat) %in% unique(clust$V3)]
env.std<-t(apply(env_keep,1,std.by.mean))
write.table(env.std,"bayenv/noFL/env_data_noFL.txt",sep='\t',quote=FALSE,col.names=FALSE,row.names=FALSE)
```
