...
We began by plotting the tree tree without a root and lacking migration edges.
...
Inspecting the un-rooted tree (Figure \@ref(fig:treemixPrep)), we decided to continue with the Treemix analysis with the FLPB as root. We tested 0 through 5 migration edges and compared the residuals, trees, and migration edges. For each one of these models we ran one replicate to generate the maximum likelihood tree and ten bootstrap replicates. We then inspected the residual plots for all replicates of each model (Figures ...) and the trees with the migration edges plotted (Figures ...) to identify consistent patterns across the bootstrap replicates. After concatenating all bootstrap trees into a single tree file using a custom R script relying on the R package ape [@paradisApeEnvironmentModern2018], we generated consensus trees using SumTrees [@sukumaranSumTreesPhylogeneticTree2015] from the Dendropy python library [@sukumaranDendroPyPythonLibrary2010].
