...
Inspecting the un-rooted tree (Figure \@ref(fig:treemixPrep)), we decided to continue with the Treemix analysis with the FLPB as root. We tested 0 through 5 migration edges and compared the residuals, trees, and migration edges. We plotted all of the rooted trees with the unrooted tree for comparison (Figure \@ref(fig:plotTreemixTrees)) and their residuals (Figure \@ref(fig:TreemixPlotResids)).  

```{r plotTreemixTrees, out.width='95%',fig.cap="The Treemix trees for each model. Each column shows a different model (no root, FLPB as root but no migration edge, and then 1 through 5 migration edges with FLPB as root) and each row is a bootstrap replicate."}
knitr::include_graphics('../fwsw_results/treemix/migration_trees_treemix_FLPB.png')
```

The plots of the various models show some consistency. In most of them, a migration edge leading from the other Florida branches to the FLAB population exists (Figure \@ref(fig:plotTreemixTrees)). Most other migration edges are either between geographically nearby populations (for example FLCC->FLPB) or are indicative of gene flow between freshwater populations.   


```{r TreemixPlotResids,fig.cap="The Treemix residual plots. Each column is a different model (no root, FLPB as root but no migration edge, and then 1 through 5 migration edges with FLPB as root), and each row is a different bootstrap replicate.",out.width='95%'}
knitr::include_graphics('../fwsw_results/treemix/treemix-residuals_FLPB.png')
```



Inspecting the scaled residuals for each tree allowed us to visualize likely admixture events relative to the best-fit tree for each analysis with different migration edges. Those population pairs with residuals above zero are more closely related to each other in the data than in the best-fit tree, so are candidates for admixture events. We observed that the models with different migration edges had rather different magnitudes of residuals, with the largest magnitudes of residuals observed in the model with two migration edges, largely driven by a large residual in the FLFW-FLFW relationship (Figure \@ref(fig:TreemixPlotResids)), which we interpreted as suggesting that this population is more isolated from the other populations than represented in the best-fit tree.  
