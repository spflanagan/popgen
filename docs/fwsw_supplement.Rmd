...
```{r treemixConsensus, results='hide', fig.show='true',fig.cap="The consensus tree from running Treemix any migration edges and without a root (left) and without migration edges but FLAB set as root (right)."}
...
plot(rt, main="consensus with FLAB set as root")
```
...
```{r treemixCompare, out.width='95%',fig.cap="The plot of the tree with FLAB as root but no migration edges (left) compared to the tree with FLAB as root and two migration edges. The drift parameter is plotted on the x-axis, and migration edges are colored based on the migration weight."}
