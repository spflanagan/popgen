...
```{r optM,message=FALSE,results='hide',warning=FALSE}
library(OptM)
tmOpt<-optM("treemix/migrations/",method = 'linear')
```
```{r plotOptM,fig.keep='true',results='hide',message=FALSE,warning=FALSE,fig.cap="Scree plot showing the comparison of Treemix number of migration edges."}
```
...
kable(starTab,"latex",booktabs=TRUE,
...
par(mfrow=c(1,3),mar=c(1,1,1,2),oma=c(1,1,1,2),xpd=TRUE)
...
```{r treemixCompare, out.width='80%',fig.cap="The plot of the tree with FLAB as root but no migration edges (left) compared to the tree with FLAB as root and two migration edges. The drift parameter is plotted on the x-axis, and migration edges are colored based on the migration weight."}
