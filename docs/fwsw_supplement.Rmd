...
We began by plotting the tree without a root and lacking migration edges.

```{r treemixPrep, results='hide', fig.show='true',fig.cap="The population tree from running Treemix any migration edges (left) and the resulting residuals (right). No populations were specified as root in this analysis."}
...
Inspecting the un-rooted tree (Figure \@ref(fig:treemixPrep)), we decided to continue with the Treemix analysis with the FLPB as root. We tested 0 through 5 migration edges and compared the residuals, trees, and migration edges. For each one of these models we ran one replicate to generate the maximum likelihood tree and ten bootstrap replicates. We then inspected the residual plots for all replicates of each model (Figures ...) and the trees with the migration edges plotted (Figures ...) to identify consistent patterns across the bootstrap replicates. After concatenating all bootstrap trees into a single tree file using a custom R script relying on the R package ape [@paradisApeEnvironmentModern2018], we generated consensus trees using SumTrees [@sukumaranSumTreesPhylogeneticTree2015] from the Dendropy python library [@sukumaranDendroPyPythonLibrary2010]. >>This needs to be updated<<



```{r TreemixPlotResids, results='hide', eval=FALSE}
# visualize residuals
png("treemix/treemix-residuals_FLPB.png",height=12,width=8,units="in",res=300)
par(mfrow=c(11,7),mar=c(2,3,1,1),oma=c(1.5,1.5,1,1))
t0<-plot_resid("treemix/fwsw_k100b",  "treemix/poplist",cex = 0.75)
...
for(i in 1:10){ 
  t0<-plot_resid(paste0("treemix/fwsw_k100b_",i),  "treemix/poplist") 
  r0<-plot_resid(paste0("treemix/fwsw_k100bFLPBr_",i),  "treemix/poplist") 
  r1<-plot_resid(paste0("treemix/fwsw_k100bFLPBrm1_",i),  "treemix/poplist") 
  r2<-plot_resid(paste0("treemix/fwsw_k100bFLPBrm2_",i),  "treemix/poplist") 
  r3<-plot_resid(paste0("treemix/fwsw_k100bFLPBrm3_",i),  "treemix/poplist") 
  r4<-plot_resid(paste0("treemix/fwsw_k100bFLPBrm4_",i),  "treemix/poplist") 
  r5<-plot_resid(paste0("treemix/fwsw_k100bFLPBrm5_",i),  "treemix/poplist") 
}
dev.off()
```


```{r residPlots,fig.cap="The Treemix residual plots. Each column is a different model (no root, FLPB as root but no migration edge, and then 1 through 5 migration edges with FLPB as root), and each row is a different bootstrap replicate.",out.width='95%'}
knitr::include_graphics('../fwsw_results/treemix/treemix-residuals_FLPB.png')
```


The plots of the various models show some consistency. In most of them, a migration edge leading from the other Florida branches to the FLAB population exists (Figure \@ref(fig:TreemixTrees)). Most other migration edges are either between geographically nearby populations (for example FLCC->FLPB) or are indicative of gene flow between freshwater populations. 

```{r plotTreemixTrees, results='hide', eval=FALSE}
# look at the trees
png("treemix/migration_trees_treemix_FLPB.png",height=12,width=8,units="in",res=300)
par(mfrow=c(11,7),mar=c(1,1,1,1),oma=c(1,1,1,1))
r0<-plot_tree("treemix/fwsw_k100b",plus=0.05,plotmig = F,scale=T,mbar=F,cex = 0.75)
t0<-plot_tree("treemix/fwsw_k100bFLPBr",plotmig = F,plus=0.05,scale=T,mbar=F,cex = 0.75)
t1<-plot_tree("treemix/fwsw_k100bFLPBrm1",plus=0.05,scale=T,mbar=T,cex = 0.75)
t2<-plot_tree("treemix/fwsw_k100bFLPBrm2",plus=0.05,scale=T,mbar=F,cex = 0.75)
t3<-plot_tree("treemix/fwsw_k100bFLPBrm3",plus=0.05,scale=T,mbar=F,cex = 0.75)
t4<-plot_tree("treemix/fwsw_k100bFLPBrm4",plus=0.05,scale=T,mbar=F,cex = 0.75)
t5<-plot_tree("treemix/fwsw_k100bFLPBrm5",plus=0.05,scale=T,mbar=F,cex = 0.75)

for(i in 1:10){ 
  r0<-plot_tree(paste0("treemix/fwsw_k100b_",i),plus=0.05,plotmig = F,scale=T,mbar=F,cex = 0.75) 
  t0<-plot_tree(paste0("treemix/fwsw_k100bFLPBr_",i),scale=T,mbar=F,cex = 0.75,lwd=2,disp=0.0002,scadj=0.05) 
  t1<-plot_tree(paste0("treemix/fwsw_k100bFLPBrm1_",i),scale=T,mbar=F,cex = 0.75,lwd=2,disp=0.0002,scadj=0.05) 
  t2<-plot_tree(paste0("treemix/fwsw_k100bFLPBrm2_",i),scale=T,mbar=F,cex = 0.75,lwd=2,disp=0.0002,scadj=0.05) 
  t3<-plot_tree(paste0("treemix/fwsw_k100bFLPBrm3_",i),scale=T,mbar=F,cex = 0.75,lwd=2,disp=0.0002,scadj=0.05) 
  t4<-plot_tree(paste0("treemix/fwsw_k100bFLPBrm4_",i),scale=T,mbar=F,cex = 0.75,lwd=2,disp=0.0002,scadj=0.05) 
  t5<-plot_tree(paste0("treemix/fwsw_k100bFLPBrm5_",i),scale=T,mbar=F,cex = 0.75,lwd=2,disp=0.0002,scadj=0.05) 
}
dev.off()
```


```{r TreemixTrees, out.width='95%',fig.cap="The Treemix trees for each model. Each column shows a different model (no root, FLPB as root but no migration edge, and then 1 through 5 migration edges with FLPB as root) and each row is a bootstrap replicate."}
knitr::include_graphics('../fwsw_results/treemix/migration_trees_treemix_FLPB.png')
```



Inspecting the scaled residuals for each tree allowed us to visualize likely admixture events relative to the best-fit tree for each analysis with different migration edges. Those population pairs with residuals above zero are more closely related to each other in the data than in the best-fit tree, so are candidates for admixture events. We observed that the models with different migration edges had rather different magnitudes of residuals, with the largest magnitudes of residuals observed in the model with two migration edges, largely driven by a large residual in the FLFW-FLFW relationship (Figure \@ref(fig:residPlots)), which we interpreted as suggesting that this population is more isolated from the other populations than represented in the best-fit tree.  

### Choosing the optimal number of migration edges

To choose the optimal number of migration edges, we used the R package optM [@fitakOptMPackageOptimize2019]. Using the scree plot and the Evanno method, we determined that the best number of migration edges was 2 (Figure \@ref(fig:plotOptM)). 

```{r optM,message=FALSE}
library(OptM)
tmOpt<-optM("treemix")
```
```{r plotOptM,fig.keep='true',fig.cap="Scree plot showing the comparison of Treemix number of migration edges."}
...
```{r MaketreemixCompare,results='hide',eval=FALSE}
...
```{r treemixCompare, out.width='80%',fig.cap="The plot of the tree with FLPB as root but no migration edges (left) compared to the tree with FLPB as root and two migration edges. The drift parameter is plotted on the x-axis, and migration edges are colored based on the migration weight."}
