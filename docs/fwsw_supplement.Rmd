...
threepop$f3_p<-pnorm(as.numeric(as.character(threepop$f3_z))) # one sided
...
The migration edge we investigated indicated potential migration from the ancestral Florida branch to the FLPB branch. We first investigated all of the three-population trees with FLPB and FLAB and found that none of these f3 statistics are zero (Figure \@ref(fig:f3Edge1)), which suggests that FLPB is not admixed with the other Florida populations 

```{r f3Edge1,fig.pos='H',fig.width=8,fig.height=6,fig.cap="Plots of the f3 statistic (with error bars showing the standard errors) and the z-scores for each tree pointing to FLPB from a branch containing FLAB. "}
...
par(mfrow=c(2,1),oma=c(5.5,4,1,0))
...
# plot the z-scores
ptfill<-rep(1,nrow(FLPB_edge))
ptfill[which(FLPB_edge$f3_p<=0.05)]<-19
plot(1:nrow(FLPB_edge),as.numeric(FLPB_edge$f3_z),axes=FALSE,
     xlab="",ylab="f3 z-score",pch=ptfill,cex=2)
axis(2,las=1)
axis(1,pos=-10,at=1:nrow(FLPB_edge),
     labels = FLPB_edge$pops,las=2,cex.axis=0.75)

```

To investigate the results of the fourpop analysis for this migration edge, we focused on four-population trees including both FLAB and FLPB and only other Florida populations (Table \@ref(tab:f4Edge1)). The f4 statistics were very close to zero and most have standard errors overlapping zero, indicating that gene flow is not particularly likely.
