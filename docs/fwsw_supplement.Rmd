...
```{r admixK2,fig.height=1,fig.width=8,fig.cap="Admixture plot for K=2. The colors represent different genetic populations. 'FLLG' is the freshwater Florida population ('FLFW' in  main text)."}
admixK2<-admixPlotting("admixture/fwsw_all_filt.2.Q",2)
```

```{r admixK3,fig.height=1,fig.width=8,fig.cap="Admixture plot for K=3. The colors represent different genetic populations. 'FLLG' is the freshwater Florida population ('FLFW' in  main text)."}
admixK3<-admixPlotting("admixture/fwsw_all_filt.3.Q",3)
```
```{r admixK4,fig.height=1,fig.width=8,fig.cap="Admixture plot for K=4. The colors represent different genetic populations. 'FLLG' is the freshwater Florida population ('FLFW' in  main text)."}
admixK4<-admixPlotting("admixture/fwsw_all_filt.4.Q",4)
```

```{r admixk5,fig.height=1,fig.width=8,fig.cap="Admixture plot for K=5. The colors represent different genetic populations. 'FLLG' is the freshwater Florida population ('FLFW' in  main text)."}
admixK5<-admixPlotting("admixture/fwsw_all_filt.5.Q",5)
write.table(admixK5,"admixture/admixK5.txt",sep = '\t',
            quote = FALSE,col.names = TRUE,row.names = FALSE)
```

```{r admixk6,fig.height=1,fig.width=8,fig.cap="Admixture plot for K=6. The colors represent different genetic populations. 'FLLG' is the freshwater Florida population ('FLFW' in  main text)."}
admixK6<-admixPlotting("admixture/fwsw_all_filt.6.Q",6)
```

```{r admixk7,fig.height=1,fig.width=8,fig.cap="Admixture plot for K=7. The colors represent different genetic populations. 'FLLG' is the freshwater Florida population ('FLFW' in  main text)."}
...
```{r treemixConsensus, results='hide', fig.show='true',fig.cap="The consensus tree from running Treemix any migration edges and without a root (left) and without migration edges but FLAB set as root (right).'FLLG' is the freshwater Florida population ('FLFW' in  main text)."}
...
par(mfrow=c(1,2),oma=c(2,2,2,2),mar=c(2,2,2,2),xpd=TRUE)
plot(ur, main="unrooted consensus")
plot(rt, main="consensus with 
FLAB set as root")
```
...
```{r treemixMigrations, message=FALSE,results='hide',fig.height=10,fig.width=5,fig.cap="Residuals (left) and trees of the treemix runs with migration edges. All models used the consensus tree with FLAB as root. 'FLLG' is the freshwater Florida population ('FLFW' in  main text)."}
...
kable(summary(as.factor(stops)),"latex",booktabs=TRUE,
      ...
      caption="Number of bootstraps with the one migration edge ending at each of these points on the population trees. 'FLLG' is the freshwater Florida population ('FLFW' in  main text).")%>%
  kable_styling(latex_options=c("HOLD_position"))
```

The overwhelming majority of bootstrapped trees with one migration edge have that edge leading from the main branch of the tree to FLPB (see Tables \@ref(tab:M1Starts) and \@ref(tab:M1Stops) for the common start and end points). The maximum likelihood tree with one migration edge -- and also the highest likelihood bootstrapped tree with one migration edge -- had a similar tree structure to the rooted tree with no migration edges, with the Atlantic coast of Florida, the Gulf coast of Florida forming another cluster, the Alabama and Louisiana populations, and the Texas populations each forming their own clusters, and FLLG (which is the Florida freshwater population) branching off between the two Florida clusters (Figure \@ref(fig:treemixCompare)). 
...
```{r treemixCompare, out.width='95%',fig.cap="The plot of the tree with FLAB as root but no migration edges (left) compared to the tree with FLAB as root and two migration edges. The drift parameter is plotted on the x-axis, and migration edges are colored based on the migration weight. 'FLLG' is the freshwater Florida population ('FLFW' in  main text)."}
...
To investigate the results of the fourpop analysis for this migration edge, we focused on four-population trees including both FLAB and FLPB and other Florida populations (Table \@ref(tab:f4Edge1)). All of these four population trees show evidence of admixture (Figure \@ref(fig:f4Edge1Fig)), which is unsurprising given that these trees include populations that are in the same population clusters in the \@ref(admixture) and \@ref(pcadapt) results.
...
kable(f4s,"latex",booktabs=TRUE,longtable=TRUE,
      caption = "Four-population trees with FLPB and FLAB and their f4 statistic, standard error, z-score, and p-value. 'FLLG' is the freshwater Florida population ('FLFW' in  main text)." )%>%
  kable_styling(latex_options=c("HOLD_position","repeat_header"))
```

```{r f4Edge1Fig,fig.pos='H',fig.width=8,fig.height=6,fig.cap="Plot of the f4 statistic for four-population trees including both FLPB and FLAB and other Florida populations. Error bars show the standard errors generated from block jackknifes. Trees indicative of admixture in the tree are those with significantly non-zero f4 statistics. 'FLLG' is the freshwater Florida population ('FLFW' in  main text)."}
