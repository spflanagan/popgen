...
all.shared<-fl.sig[which(fl.sig %in% la.sig & fl.sig %in% al.sig & fl.sig %in% tx.sig)]
gulf_shared<-tx.sig[which(tx.sig %in% la.sig & tx.sig %in% al.sig)]
```

There are `r length(unique(all.shared))` outliers (as determined by Fisher's P from stacks < 0.05). But because of the large pairwise Fsts between the Florida populations (Figure \@ref(fig:plotStacksFsts)), we'll focus on the shared SNPs in the Texas, Alabama, and Louisiana analyeses, in which we have `r length(unique(gulf_shared))`. 
