...
```{r loadplink,cache=TRUE}
ped.sub<-read.table("stacks/populations_all/batch_2.plink.ped",header=F)	
ped.sub$V1<-gsub("sample_(\\w{4})\\w+.*","\\1",ped.sub$V2)
map.sub<-read.table("stacks/populations_all/batch_2.plink.map",header = F,stringsAsFactors = F)
