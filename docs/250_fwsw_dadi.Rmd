...
sumTab<-sumTab[,-c(9,10,11)]  

landscape(kable(sumTab,"latex",booktabs=TRUE,longtable=TRUE,
...
```
```{r setupOutTable}
best_mod<-names(which.max(tapply(as.numeric(fllg_flcc[[1]]$log.likelihood),fllg_flcc[[1]]$Model,median)))
outNames<-colnames(do.call(rbind,
                           extract_dadi_params(
                             fllg_flcc[[1]][fllg_flcc[[1]]$Model==best_mod,])))
...
The best model appears to be `r best_mod`.
...
```
```{r BestMods2}
best_mod<-names(which.max(tapply(as.numeric(fllg_alfw[[1]]$log.likelihood),fllg_alfw[[1]]$Model,median)))
...
```
```{r BestMods3}
best_mod<-names(which.max(tapply(as.numeric(fllg_alst[[1]]$log.likelihood),fllg_alst[[1]]$Model,median)))
...
```
```{r BestMods4}
best_mod<-names(which.max(tapply(as.numeric(fllg_lafw[[1]]$log.likelihood),fllg_lafw[[1]]$Model,median)))
...
```
```{r BestMods5}
best_mod<-names(which.max(tapply(as.numeric(fllg_txfw[[1]]$log.likelihood),fllg_txfw[[1]]$Model,median)))
...
```
```{r BestMods6}
best_mod<-names(which.max(tapply(as.numeric(fllg_txcc[[1]]$log.likelihood),fllg_txcc[[1]]$Model,median)))
...
```
```{r BestMods7}
best_mod<-names(which.max(tapply(as.numeric(flcc_alfw[[1]]$log.likelihood),flcc_alfw[[1]]$Model,median)))
...
```
```{r BestMods8}
best_mod<-names(which.max(tapply(as.numeric(flcc_alst[[1]]$log.likelihood),flcc_alst[[1]]$Model,median)))
...
```
```{r BestMods9}
best_mod<-names(which.max(tapply(as.numeric(flcc_lafw[[1]]$log.likelihood),flcc_lafw[[1]]$Model,median)))
...
```
```{r BestMods10}
best_mod<-names(which.max(tapply(as.numeric(flcc_txfw[[1]]$log.likelihood),flcc_txfw[[1]]$Model,median)))
...
```
```{r BestMods11}
best_mod<-names(which.max(tapply(as.numeric(flcc_txcc[[1]]$log.likelihood),flcc_txcc[[1]]$Model,median)))
...
```
```{r BestMods12}
best_mod<-names(which.max(tapply(as.numeric(alfw_alst[[1]]$log.likelihood),alfw_alst[[1]]$Model,median)))
...
```
```{r BestMods13}
best_mod<-names(which.max(tapply(as.numeric(alfw_lafw[[1]]$log.likelihood),alfw_lafw[[1]]$Model,median)))
...
```
```{r BestMods14}
best_mod<-names(which.max(tapply(as.numeric(alfw_txfw[[1]]$log.likelihood),alfw_txfw[[1]]$Model,median)))
...
```
```{r BestMods15}
best_mod<-names(which.max(tapply(as.numeric(alfw_txcc[[1]]$log.likelihood),alfw_txcc[[1]]$Model,median)))
...
```
```{r BestMods16}
best_mod<-names(which.max(tapply(as.numeric(alst_lafw[[1]]$log.likelihood),alst_lafw[[1]]$Model,median)))
...
```
```{r BestMods17}
best_mod<-names(which.max(tapply(as.numeric(alst_txfw[[1]]$log.likelihood),alst_txfw[[1]]$Model,median)))
...
```
```{r BestMods18}
best_mod<-names(which.max(tapply(as.numeric(alst_txcc[[1]]$log.likelihood),alst_txcc[[1]]$Model,median)))
...
```
```{r BestMods19}
best_mod<-names(which.max(tapply(as.numeric(lafw_txfw[[1]]$log.likelihood),lafw_txfw[[1]]$Model,median)))
...
```
```{r BestMods20}
best_mod<-names(which.max(tapply(as.numeric(lafw_txcc[[1]]$log.likelihood),lafw_txcc[[1]]$Model,median)))
...
```
```{r BestMods21}
best_mod<-names(which.max(tapply(as.numeric(txfw_txcc[[1]]$log.likelihood),txfw_txcc[[1]]$Model,median)))
