...
```{r}
tx.opts<-dadi.modelcomp(path = "dadi_analysis/TX2D",pattern=".*optimized.*",id="TX2D")
kable(tx.opts,caption = "'Best' runs of each of the models for the Texas populations 2D demographic inference")
```
...
```{r}
tx.nans<-do.call(rbind,lapply(list.files(path = "dadi_analysis/TX2D",pattern=".*optimized.*",full.names = TRUE),dadi.nan))
```
