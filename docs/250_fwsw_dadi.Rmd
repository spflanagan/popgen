...
```{r fllgCompareMods,fig.width=8, fig.cap="Boxplot showing the parameter estimates from the replicates of the SI model (left) and the IM model (right) for the FLFW vs FLCC pairwise comparison. The red stars represent the model parameters from the run with the highest log likelihood.", eval=FALSE}
par(mfrow=c(1,2))
...
```{r afasOpts, fig.cap="Comparison of log likelihoods for the simple ALFW-ALST models"}
