...
```{r}
fl_dat<-extract_dadi_params(fl.dat)
...
dadi_warnings<-function(logname, mods=NA){
 ...
 # browser()
  ...
  if(length(mods)>1 | !is.na(mods)){ models<-models[models %in% mods] }
...
Nope! The first one had `r dim(tx.warnings[tx.warnings$Warning=="TRUE",])` and the second had `r dim(tx.warnings2[tx.warnings2$Warning=="TRUE",])` warnings. I think it might be time to try the other type of model. 
